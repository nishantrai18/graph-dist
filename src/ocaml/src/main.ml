...
let coms = [
  ...
  (fun () ->
    ...
    for u=0 to G.index_max !the_graph do
      Debug.distr "deg_dist" (G.degree !the_graph u) ;
    done ;
  ...
  "reverse", ": reverse edges orientation of the graph",
  (fun () ->
    let g = !the_graph in
    the_graph := G.reverse g ;
    Debug.info "Reverse n=%d m=%d" (G.n !the_graph) (G.m !the_graph)
  );
  ...
  (fun () ->
    ...
    let diam, rad, rad', _ = G.diameter_scc g u in
    Printf.printf "%d %d %d\n" diam rad rad' ; flush stdout ;
    Debug.info "Misc diameter of scc of %s : %d" (G.name g u) diam
  );

  "scc_sizes", ": compute the sizes of the strongly connected \
               component",
  (fun () ->
    let g = !the_graph in
    let ccs = G.strongly_connected_components g in
    let size = G.component_sizes ccs in
    for c = 0 to Array.length size - 1 do
  ...
  (fun () ->
    ...
    let print u = 
      let uname = int_of_string (G.name g u) in
      let lon, lat = Coord.get_loc coord uname in
  ...
  "diameters", "[coord_file] [min_size] : compute the diameter of each \
                strongly connected component with size [min_size] at least \
                and output the corresponding paths.",
  (fun () ->
    let g = !the_graph in
    let coord = TrivialArg.string "coord_file" in
    let min_size = TrivialArg.int "min_size" in
    let coord = dimacs_coordinates int_of_string coord in
    let ccs = G.strongly_connected_components g in
    let first = ref true in
    let i_comp = ref 0 and n_comp = G.component_total_number ccs in
    G.component_iter (fun comp_nb u comp_size ->
      if comp_size >= min_size then begin
        incr i_comp ;
        Debug.info "------------ Component %d / %d : %d  size %d --------------"
          !i_comp n_comp comp_nb comp_size ;
        let g,u,in_scc =
          if comp_size > (G.n g) / 10 then 
            g, u, (fun u -> G.component_number ccs u = comp_nb)
          else begin
            G.mark_component g ccs comp_nb ;
            G.induced_subgraph g, 0, (fun _ -> true)
          end
        in
        let diam, rad, rad', (sw_d, _, _, _) = 
          G.diameter_scc ~diam_only:true g u in
        let s = sw_d.G.node and t = sw_d.G.last in
        let dist, parent, last = G.dijkstra g s in
        Debug.info "diameters of scc of %s : %d from %s to %s (%d to %s)" 
          (G.name g u) diam (G.name g s) (G.name g t) 
          (G.V.label dist last) (G.name g last) ;
        let print_branch u root =
          let print u = 
            let uname = int_of_string (G.name g u) in
            let lon, lat = Coord.get_loc coord uname in
          ...
          let rec iter acc u = 
            if u = root then begin
              if !first then (Printf.printf "[" ; first := false)
              else Printf.printf ",\n[" ;
              print u ;
              List.iter (fun u -> Printf.printf "," ; print u) acc ;
              Printf.printf "]" ;
              ...
              let p = G.V.index parent u in
              let acc = if in_scc u then u :: acc else acc in
              iter acc p
            end
          in
          iter [] u ;
        in
        if s <> 0 then print_branch last s
      end
    ) ccs ;
  );
  ...
  "where", "[node_name] [coord_file] : return the lat and long of the node.",
  (fun () ->
    let node = TrivialArg.string "node_name" in
    let coord = TrivialArg.string "coord_file" in
    let coord = dimacs_coordinates int_of_string coord in
    let lon,lat = Coord.get_loc coord (int_of_string node) in
    Debug.info "Where : %s %f,%f" node lat lon ;
    Printf.printf "%f %f\n" lat lon
  );
